...
public class Json_Controller : MonoBehaviour
{
    ...
    string aid_val;
    //public List<string> equipment_val = new List<string>();
    string charId;
    ...
    public void Load_json_Attributes(string jsonString)
    {
        ...
        val.initial_accuracy = root2.initialaccuracy;
    ...
    public void Load_Inventory(string json_String)
    {
        Debug.Log("start");
       ...
       var tok = JsonUtility.FromJson<Tokens>(json_String);
        var tok2 = JsonConvert.DeserializeObject<Inventory>(json_String);
        //inventory = JsonUtility.FromJson<Inventory>(json_String);
        //for (int i = 0; i < inventory.token.Count; i++)
        //{
        Debug.Log(tok.bronze);
        Debug.Log(tok2.token.bronze);
        //}
    }
    ...
    public void Save_atribute()
    {
        ...
        root2.initialaccuracy = val.initial_accuracy;
    ...
    void test_read(string jsonString)
    {
        root = JsonUtility.FromJson<Root>(jsonString);
        Debug.Log(root.equipment.Count);
        //equipment_val = root.equipment;
        val.equipment = root.equipment;
        for (int i = 0; i < root.equipment.Count; i++)
        {
            //val.equipment.Add(root.equipment[i]);
            //Debug.Log(root.equipment[i]);
            if (root.equipment[i] != "null")
            {
                //val.equipment.Add(Convert.ToInt32(root.equipment[i]));
                Debug.Log(root.equipment[i]);
            }
            else
            {
    ...
    void Start()
    {
        ...
        string test3 = File.ReadAllText(Application.streamingAssetsPath + "/Inventory_Player.json");// для теста
    ...
    public class Tokens
    {
        ...
        //public Tokens(int icp, int gold, int coal, int ore, int adit, int lgs, int leather, int bronze, int tp)
        //{
        //    this.icp = icp;
        //    this.gold = gold;
        //    this.coal = coal;
