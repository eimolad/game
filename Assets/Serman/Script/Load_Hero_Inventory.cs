...
public class Load_Hero_Inventory : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //await Task.Run(() => {  });
        //await Task.Run(() => { });
        StartCoroutine(load_hero_complekt());
    }
    public IEnumerator load_hero_complekt()
    {
        //Debug.Log(VAL.equipment.Count);

        //for (int i = 0; i < equipment.Count; i++) Debug.Log("комплект из json " + equipment[i]);
